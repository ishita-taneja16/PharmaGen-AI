...
import { ShieldAlert, CheckCircle2, AlertTriangle, FileText, Lock, Sparkles, ShieldCheck, CheckSquare } from 'lucide-react';
...
export const ComplianceChecker: React.FC = () => {
  ...
  const [report, setReport] = useState<any>({
    ...
    missing_steps: [
      {
        step_number: 4,
        requirement: 'Filter solution through 0.22 micron membrane prior to final precipitate collection.',
        severity: 'CRITICAL'
      }
    ],
    capa_recommendations: [
      'Issue automated checklist prompt to lab operators prior to filtration step',
  ...
  const handleVerify = async () => {
    setLoading(true);
    try {
      const res = await verifyCompliance('d4e5f6a7-8901-2345-6789-abcdef012345', sopCode);
      setReport(res);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="h-full flex flex-col gap-6 overflow-y-auto pr-1">
      <div>
        ...
        <p className="text-sm text-slate-400">Automated protocol verification, gap analysis, quantitative risk scoring & SHA-256 Part 11 audit trails</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="glass-panel p-5 rounded-2xl flex flex-col gap-4">
          ...
          <button
            ...
            disabled={loading}
            className="w-full py-2.5 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-medium text-sm flex items-center justify-center gap-2 transition shadow-lg shadow-rose-950"
          >
